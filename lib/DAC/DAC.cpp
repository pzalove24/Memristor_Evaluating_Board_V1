...
void DAC_bias::pulse_bias(float PositiveVoltage, float NegativeVoltage, int PositivePulseNumber=10, int NegativePulseNumber=10, 
float DelayRead_us = 50, float DelayTime_ms = 100, bool continuoes = false) {
  do {
    ...
    for (int i=0; i<PositivePulseNumber;i++ ){
      ...
      dac.setVoltage((0.1*4095)/5, false); //read
      delay(DelayTime_ms);
      }
